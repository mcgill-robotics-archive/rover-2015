			...
			else
			{
				if (vd->buf.bytesused > framesizeIn) {
					memcpy(vd->framebuffer, vd->mem[vd->buf.index],
						(size_t) framesizeIn);
//Start of e-con
				} else if(vd->buf.bytesused < framesizeIn) {
					// discard the buffer, if the image is less than the actual size
				} else {
//end of e-con
					memcpy(vd->framebuffer, vd->mem[vd->buf.index],
...
//Start of e-con
/* TO DO: e-con's See3CAM_10M_CUG_C enumerated as pixelformat 'BY8' or 'BA81', which is V4L2_PIX_FMT_SBGGR8.
	  But camera's output pixel order is GRGR... | BGBG... We need to change the pixelformat guid(format)
	  or change the output pixel order in the firmware. So we use the bayer conversion format GRBG.
*/
//End of e-con
		case V4L2_PIX_FMT_SBGGR8: //2
//Start of e-con
...
int uvcGrab(struct vdIn *vd, struct GLOBAL *global, int format, int width, int height)
{
	...
	switch(vd->cap_meth)
	{
		...
		default:
			...
			vd->buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
			vd->buf.memory = V4L2_MEMORY_MMAP;
if(0 == global->FX320_defaults_flag)
{
