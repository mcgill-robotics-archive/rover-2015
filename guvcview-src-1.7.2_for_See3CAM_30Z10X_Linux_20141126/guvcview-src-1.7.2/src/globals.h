...
//Start of e-con
    int hid_udev_timer_id;     //timer id for hid_udev device events check
    int FX320_defaults_flag;     //flag for defaults button pressed for FX320
	int FX320_scene_mode;			// 1 for normal, 2 for macro
	int FX320_focus_mode;			// 1 for continous, 2 for single trigger, 3 for manual
	int FX320_focus_position;		// Focus value (0 ~ 255)
	int FX320_brightness;			// brightness value (0 ~ 15)
	int FX320_contrast;			// contrast value (0 ~ 15)
	int FX320_shutter_speed;		// shutter speed value (0 ~ 9)
	int FX320_orientation;		// 1 for NULL, 2 for F, 3 for M, 4 for F&M
	int FX320_optical_zoom;		// optical zoom value (0 ~ 43)
	int FX320_digital_zoom;		// digital zoom value (0 ~ 15)
	int FX320_wb_mode;		// 1 for auto, 2 for daylight, 3 for cloudy, 4 for fluorescent, 5 for manual
	int FX320_red_gain;		// red gain value (0 ~ 255)
	int FX320_blue_gain;		// blue gain value (0 ~ 255)
	int FX320_temp_noise_filter;	// temporal noise filter value (0 ~ 15)
	int FX320_compression;		// compression value (0 - Fine (1/12) and 1 - Best (1/3) )
	pchar* FX320_imgFPath;       //image path [0] - filename  [1] - dir
	pchar* FX320_imgFName;		// Image file names ( destination with full path)
	pchar* FX320_imgFName_read;	// Image file names ( source with full path)
	UINT64 FX320_image_inc;    //image name increment
//End of e-con
