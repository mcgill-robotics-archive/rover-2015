...
//Start of-e-con
    if(global->width > global->desktop_w) {
...
//Start of e-con
    if(global->width > 1920) {
        if(global->height >1080)
            overlay = SDL_CreateYUVOverlay(1920, 1080, SDL_YUY2_OVERLAY, *pscreen);
        else
            overlay = SDL_CreateYUVOverlay(1920, global->height, SDL_YUY2_OVERLAY, *pscreen);
    } else
//End of e-con
    ...
    while (!signalquit)
    {
        ...
        else
        {
            ...
            {/* sets fps count in window title bar */
                ...
                { /*set every 2 sec*/
//Start of e-con
                    g_snprintf(global->WVcaption,32,"See3CAM GUVCVideo - %3.2f fps",global->DispFps);
//End of e-con
                    SDL_WM_SetCaption(global->WVcaption, NULL);
...
//Start of e-con
        /*------------------Fix for SEE3CAM_80 flash mode issue---------*/
		if(g_flash_flag && g_external_trigger_flag && video_win_flag)
		{
			g_external_trigger_flag = FALSE;
			video_win_flag = FALSE;
...
//Start of e-con
		...
		if(height > 1080) {
		    for(i = 0; i < 1080; i++)
			memcpy(p+(i*1920*2), videoIn->framebuffer+(i*width*2), 1920*2);
		} else {
		    for(i = 0; i < height; i++)
			memcpy(p+(i*1920*2), videoIn->framebuffer+(i*width*2), 1920*2);
		}
...
//End of e-con
            ...
            while( SDL_PollEvent(&event) )
            {
                ...
                {
//Start of e-con
//To disable window resize events when changing resolution of camera
#if 0
//End of e-con
                    pscreen =
