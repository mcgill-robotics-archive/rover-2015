...
writeConf(struct GLOBAL *global, char *videodevice)
{
	...
	{
		g_fprintf(fp,"# see3camguvcview configuration file for %s\n\n",videodevice);
...
if( SEE3CAM_30_Z10X == curr_see3cam_dev) {
		g_fprintf(fp, "# See3cam_z10x camera details\n");
		g_fprintf(fp, "fx320_scene_mode=%d\n",global->FX320_scene_mode);
		g_fprintf(fp, "fx320_focus_mode=%d\n",global->FX320_focus_mode);
		g_fprintf(fp, "fx320_focus_position=%d\n",global->FX320_focus_position);
		g_fprintf(fp, "fx320_brightness=%d\n",global->FX320_brightness);
		g_fprintf(fp, "fx320_contrast=%d\n",global->FX320_contrast);
		g_fprintf(fp, "fx320_shutter_speed=%d\n",global->FX320_shutter_speed);
		g_fprintf(fp, "fx320_orientation=%d\n",global->FX320_orientation);
		g_fprintf(fp, "fx320_optical_zoom=%d\n",global->FX320_optical_zoom);
		g_fprintf(fp, "fx320_digital_zoom=%d\n",global->FX320_digital_zoom);
		g_fprintf(fp, "fx320_wb_mode=%d\n",global->FX320_wb_mode);
		g_fprintf(fp, "fx320_red_gain=%d\n",global->FX320_red_gain);
		g_fprintf(fp, "fx320_blue_gain=%d\n",global->FX320_blue_gain);
		g_fprintf(fp, "fx320_temp_noise_filter=%d\n",global->FX320_temp_noise_filter);
		g_fprintf(fp, "fx320_compression=%d\n",global->FX320_compression);
		g_fprintf(fp,"# See3CAM Image capture Full Path\n");
		g_fprintf(fp,"fx320_image_path='%s/%s'\n",global->FX320_imgFPath[1],global->FX320_imgFPath[0]);
}
		printf("write %s OK\n",global->confPath);
			...
			if (ttype == G_TOKEN_STRING)
			{
				...
				else
				{
					...
					if (ttype == G_TOKEN_STRING)
					{
						...
						else if (g_strcmp0(name,"fx320_image_path")==0)
						{
								global->FX320_imgFPath = splitPath(scanner->value.v_string,global->FX320_imgFPath);
						int i;
						for(i=0;i<8;i++){
...
//							printf("FX320_imgFName[%d] : %s\n",i,global->FX320_imgFName[i]);
					...
					else if (ttype==G_TOKEN_INT)
					{
						...
						else if (g_strcmp0(name,"fx320_scene_mode")==0)
						{
							global->FX320_scene_mode = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_focus_mode")==0)
						{
							global->FX320_focus_mode = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_focus_position")==0)
						{
							global->FX320_focus_position = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_brightness")==0)
						{
							global->FX320_brightness = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_contrast")==0)
						{
							global->FX320_contrast = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_shutter_speed")==0)
						{
							global->FX320_shutter_speed = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_orientation")==0)
						{
							global->FX320_orientation = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_optical_zoom")==0)
						{
							global->FX320_optical_zoom = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_digital_zoom")==0)
						{
							global->FX320_digital_zoom = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_wb_mode")==0)
						{
							global->FX320_wb_mode = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_red_gain")==0)
						{
							global->FX320_red_gain = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_blue_gain")==0)
						{
							global->FX320_blue_gain = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_temp_noise_filter")==0)
						{
							global->FX320_temp_noise_filter = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"fx320_compression")==0)
						{
							global->FX320_compression = scanner->value.v_int;
						}
						else
						{
