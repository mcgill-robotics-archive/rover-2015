...
ERR_DIALOG(const char *err_title, const char* err_msg, struct ALL_DATA *all_data)
{
    ...
    else
    {
        ...
        if (videoIn->listDevices->num_devices > 1)
        {
            ...
            gtk_widget_override_font(title, pango_font_description_from_string ("Sans bold 10"));
...
key_pressed (GtkWidget *win, GdkEventKey *event, struct ALL_DATA *all_data)
{
    ...
    {
        case GDK_KEY_WebCam:
				g_external_trigger_flag = TRUE;
				control_win_flag = TRUE;

//Start of e-con
//To suppress continuous generation of webcam key event.
//if(key_flag)
//{
//End of e-con
...
//Start of e-con
//Key release event handler by e-con
#if 1
gboolean
key_released (GtkWidget *win, GdkEventKey *event, struct ALL_DATA *all_data)
{
	BOOL ret_val = FALSE;
    switch (event->keyval)
    {
        case GDK_KEY_WebCam:
if(curr_see3cam_dev == SEE3CAM_80USB && g_flash_flag && g_external_trigger_flag && control_win_flag) {
			g_external_trigger_flag = FALSE;
			control_win_flag = FALSE;
			ret_val=SendCaptureComplete();
			if(ret_val == FALSE)
			{
				printf("send capture complete error\n");
			}
}
//	key_flag=TRUE;
	return TRUE;
...
capture_image (GtkButton *ImageButt, struct ALL_DATA *all_data)
{
	...
	struct GLOBAL *global = all_data->global;
	struct vdIn *videoIn = all_data->videoIn;
//e-con : To make sure default image capture callback updates the correct file index before saving.
if(SEE3CAM_30_Z10X==curr_see3cam_dev){
	gtk_check_menu_item_toggled(GTK_CHECK_MENU_ITEM(gwidget->photo_timestamp));
	gtk_check_menu_item_toggled(GTK_CHECK_MENU_ITEM(gwidget->photo_timestamp));
}
...
//e-con
			/*FIXME:
				This uvcx_request_frame_type fn. causes kernel crash because of memory allocation issue. This is not required for the device which does not support h264. Hence we add a condition to check whether the current format is 'V4L2_PIX_FMT_H264' and execture the function.
			*/
